...
public class AddFriendCommand implements CommandExecutor{
	...
	public CommandResult execute(CommandSource src, CommandContext args) throws CommandException {
		...
		if (src instanceof Player) {
			...
			friend.sendMessage(Text.of(TextColors.DARK_AQUA, "You have a pending friend request from ", TextColors.GOLD, player.getName(), TextColors.DARK_AQUA, ", Type /lp acceptrequest to accept."));
			...
			Task task = taskBuilder.execute(new CancellingTask(friendRequest)).async().interval(1, TimeUnit.SECONDS).name("LandProtect Friend Request Task").submit(LandProtect.instance);
		}
